...
<html>
    <head>
        <title>Editar Perfil de Seguridad</title>
        <meta charset="UTF-8">
    ...
    <body>
        <div id="seccion01" class="seccion01">
            ...
            <div class="subtitulo_aplicacion">Editar Perfil de Seguridad</div>
        ...
        <?php
            $queryAccionesViejas = "SELECT idacciones FROM perfiles_acciones WHERE idperfil = " . $_GET["id"] . ";";
            $resultAccionesViejas = pg_query($queryAccionesViejas) or die('La consulta fallo: ' . pg_last_error());

            $valueAccMarcadas = "";
            $bandera=0;

            while ($accionesVie = pg_fetch_array($resultAccionesViejas)){
                if($bandera==0){
                    $valueAccMarcadas.= $accionesVie["idacciones"];
                    $bandera = 1;
                }else{
                    $valueAccMarcadas.= "_".$accionesVie["idacciones"];
                }
            }

           ?>
        
        <input type="hidden" id="accMarcadas" name="accMarcadas" value="<?php echo $valueAccMarcadas;?>">
        ...
        <div id="seccion04" class="seccion04">
            ...
            <div style="width: 400px; height: 40px; float: left;">
                ...
                <div class="titulo_hoja">Editar Perfil de Seguridad</div>
                <div class="subtitulo_hoja">Redición de Perfil de Seguridad</div>
            </div>
        </div>
        <div id="seccion03" class="seccion03">
            <div style="float: left; border: 1px solid #CCC; height: auto; width: 820px; margin: 15px; padding: 15px; padding-top: 0px;">
                <form id="editarPerfSeg" name="editarPerfSeg" method="post" action="recursos/php/acciones.php?accion=editarPerfilSeguridad&id=<?php echo $_GET["id"]?>">
                    
                    <?php
                        $sqlPerfilAEditar = 
                            "SELECT perfil.idperfil AS idperfil, perfil.nombre AS nombre, proveedor.idproveedor AS idproveedor, 
                        entidad.identidad AS identidad, acciones.idacciones AS idacciones
                        ...
                        WHERE perfil.idperfil = ".$_GET["id"].";";
                        
                        $resultPerfilAEditar = pg_query($sqlPerfilAEditar) or die('La consulta fallo: ' . pg_last_error());
                        
                        //Solo para los datos del nombre y el proveedor
                        $filaUno = pg_fetch_array($resultPerfilAEditar,0);
                        //Entidades array
                        $entidades = array();
                        while ($tablaPerfilEditar = pg_fetch_array($resultPerfilAEditar)){
                            $entidades[] = $tablaPerfilEditar["identidad"];
                        }
                        $entidadesUnicas = array_unique($entidades);
                        
                    ?>
                    
                    <input type="hidden" name="concatenado" id="concatenado" value="" />
                    <input type="hidden" name="idperfil" id="idperfil" value="<?php echo $_GET["id"]?>" />
                    ...
                    <div class="formularioAgregaGRUPO">
                         ...
                         <div class="ConfiguracionElemento"  style="margin-bottom: 5px;">
                            ...
                            <div class="ConfiguracionElemento-contenedor">
                                <input id="nombrePerfil" name="nombrePerfil" type="text" class="entradaConfiguracion" value="<?php echo $filaUno["nombre"]?>"/>
                            </div>
                            ...
                            <div class="ConfiguracionElemento-contenedor">
                                <select onchange='actualizaOpciones()' class="selectConfiguracion" id="selproveedor" name="selproveedor">
                                    ...
                                    while ($proveedor = pg_fetch_array($result_proveedor)) {
                                        ...
                                        if($proveedor["idproveedor"] == $filaUno["idproveedor"]){
                                            $idtemProveedor = $proveedor["idproveedor"];
                                            echo "<option selected='selected' value='" . $proveedor["idproveedor"] . "'>" . $proveedor["nombre"] . "</option>";
                                        } else {
                        ...
                        <div id="listaEntidades" class="ConfiguracionElemento"  style="margin-bottom: 10px; margin-top: 10px">
                            <fieldset>
                                ...
                                <?php
                                    $query_entidades = "SELECT * FROM entidad WHERE idproveedor = ".$idtemProveedor." order by nombre asc;";
                                    ...
                                    while ($entidad = pg_fetch_array($result_entidades)) {
                                        ...
                                        echo "<div>";
                                        echo "<input class='elementoCheckEntidad' type='checkbox' id='check".$entidad["nombre"]."' name='checkEntidades[]' value='".$entidad["identidad"]."' ".((in_array($entidad["identidad"], $entidadesUnicas)==true)? "checked":"").">";
                        ...
                        <div style="width: 700px; float: left">
                            <div onclick="editarPerfil()" class="botoncrear">Editar Perfil</div>
                        </div>
        ...
        <script type="text/javascript">
            ...
            function actualizaAcciones(){
                ...
                $.ajax({
                    ...
                    data: {accion: 56, selectedEntidades: entidades, idproveedor:actualprov, idperfil:perfil, acciones:marcadas}, //--> send id of checked checkbox on other page
            ...
            function actualizaOpciones() {
                ...
                $.ajax({
                    data: {accion: 55, idproveedor: actualprov},
                    url: './recursos/php/ajax.php',
                    type: 'post',
                    success: function (response) {
...
//                }
                    ...
                    document.getElementById("editarPerfSeg").submit();
